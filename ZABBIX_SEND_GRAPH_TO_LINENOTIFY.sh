#!/bin/bash
ZBX_URL="http://IP-SERVER ZABBIX"
USERNAME="zabbixuser"    	# e.g.: Admin
PASSWORD="zabbixpassword"   # e.g.: zabbix
PERIOD=10800
ZABBIXVERSION34="1"
GRAPHID=$3
...
if [ $(($INC_GRAPH)) -eq '1' ]; then
 ...
 ${CURL} -X POST -H "Authorization: Bearer LINE NOTIFY TOKEN" -F "message=$2" -F "imageFile=@${PNG_PATH}" https://notify-api.line.me/api/notify
else
 ${CURL} -X POST -H "Authorization: Bearer LINE NOTIFY TOKEN" -F "message=$2" https://notify-api.line.me/api/notify
fi
rm -f ${COOKIE}
rm -f ${PNG_PATH}
